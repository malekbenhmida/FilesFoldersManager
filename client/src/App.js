...
function App() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/files`,{headers: {
  ...
  const deleteFile = (nameFile, dirFile) => {
    ...
    axios.delete(`http://localhost:8080/deleteFile`,delFile,
    {headers: {
      "Access-Control-Allow-Origin" : "*"
...
const createFolder = () => {
  ...
  axios.post(`http://localhost:8080/folder`, data, {headers:{
    'content-type':'application/json',
    'Access-Control-Allow-Origin': "*"}})
  .then((response) => {
    console.log(response)
      ...
      {getfolders.filter(item=>{
          ...
          <div className="col-md-2 item">
            ...
            <i onClick={()=>setGoToFolder(items.dir.split('/')[1] === '' ? items.dir.replace('/','')  + "/" +  items.name : items.dir  + "/" +  items.name)} className="fa-solid fa-folder"></i>
              ...
              <i onClick={()=>deleteFolder(items.name, items.dir)} class="fa-solid fa-trash-can"></i>              
        ...
        {getfiles.filter(item=>{
          ...
          <div className="col-md-2 item">
            <div>
              ...
              <i onClick={()=>deleteFile(items.name, items.dir)}  class="fa-solid fa-trash-can"></i>              
              <i class="fa-solid fa-square-pen"></i>
            </div>
